...
public class customerLocation extends AppCompatActivity implements OnMapReadyCallback, TaskLoadedCallback, Listener, DirectionUtil.DirectionCallBack  {
    GoogleMap mMap;
    MarkerOptions customer_loc,dispatch_loc;
    ...
    private double customer_loc_lat,customer_loc_lng,dispatch_loc_lat,dispatch_loc_lng;
    private int i,count;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //Build the map
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager().findFragmentById(R.id.customerLocation_map);
        mapFragment.getMapAsync(customerLocation.this);

        //TODO: Get current location of device
        //Dispatch Marker
      //  dispatch_loc = new MarkerOptions().position(new LatLng(5.618708, -0.071986)).title("Dispatch Location");
    }
    ...
    public void onTaskDone(final ArrayList<LatLng> values) {
        if (values != null) {
           ...
           // markerAnimation.animateLine(values,mMap,dispatch_location,customerLocation.this,"Have you delivered the order","Call Customer","customer",getIntent().getStringExtra("store"),loc_lat,loc_lng);
            ...
            if (mLocationPermissionGranted) {
                ...
                RestClient.get("http://yomamas.de/api/orders?_format=json",null,new JsonHttpResponseHandler(){
                    ...
                    public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                       for (int i=0;i<response.length();i++){
                           try {
                               ...
                               if(o.getString("store_id").equals(getIntent().getStringExtra("store"))){

                                   if (!TextUtils.isEmpty(o.getString("field_del_location"))){
                                       String locs = o.getString("field_del_location");
                                       //Remove the all /n
                                       ...
                                       customer_loc_lat= Double.parseDouble(latlng_array[0]);
                                       customer_loc_lng= Double.parseDouble(latlng_array[1]);
                                       //Log.d("lng",String.valueOf(customer_loc_lng));

                                       //Customer Location marker
                                       customer_loc= new MarkerOptions().position(new LatLng(customer_loc_lat, customer_loc_lng)).title("Store Location");
                                       mMap.addMarker(customer_loc);
                                       ...
                                       if (count==0){
                                           ...
                                           String url = getUrl(new LatLng(dispatch_loc_lat,dispatch_loc_lng), customer_loc.getPosition(), "driving");
                                           new FetchURL(customerLocation.this).execute(url, "driving");
                                       ...
                                       Toast.makeText(customerLocation.this,"Delivery Location not Found. Enter Delivery Location on Order",Toast.LENGTH_LONG);
                    ...
                    public void onFailure(int statusCode, Header[] headers, Throwable throwable, JSONArray errorResponse) {
                        Toast.makeText(customerLocation.this,"Check Internet Connection and try again",Toast.LENGTH_LONG);
    ...
    public void currentLocation(Location location) {
        ...
        Log.d("store_latCurrent",String.valueOf(customer_loc_lat));
        ...
        MarkerAnimation markerAnimation = new MarkerAnimation();
        markerAnimation.animateLine(location.getLatitude(),location.getLongitude(), mMap, dispatch_location, customerLocation.this, "Have you delivered the product?", "Call Customer", "customer", getIntent().getStringExtra("store"), customer_loc_lat, customer_loc_lng);
